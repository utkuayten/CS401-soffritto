...
class PatchTST_backbone(nn.Module):
    def __init__(
            ...
            verbose:bool=False,
            # ------------------ NEW ------------------
            c_out: Optional[int] = None,                 # desired output channels (e.g., 16)
            legacy_output_shape: bool = False,           # if True, keep [bs, nvars, target_window]
            # -----------------------------------------
            **kwargs
    ):
        super().__init__()
        ...
        # ------------------ NEW: channel projection config ------------------
        self.c_out = c_out if c_out is not None else self.n_vars    # default: mirror inputs
        self.legacy_output_shape = legacy_output_shape

        # project from n_vars -> c_out over the "channel" axis after the head
        # (we’ll permute to [bs, target_window, n_vars] then apply Linear(n_vars -> c_out))
        self.channel_proj = None
        if self.c_out != self.n_vars:
            self.channel_proj = nn.Linear(self.n_vars, self.c_out, bias=True)
        # --------------------------------------------------------------------
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone(z)                                               # [bs x nvars x d_model x patch_num]
        z = self.head(z)                                                   # [bs x nvars x target_window]

        # -------------- NEW: shape & channel projection --------------
        # Legacy users can keep the original shape:
        if self.legacy_output_shape and self.c_out == self.n_vars:
