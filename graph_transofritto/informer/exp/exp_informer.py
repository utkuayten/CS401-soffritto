...
from graph_transofritto.informer.data_loader.data_loader import Dataset_Custom
from graph_transofritto.informer.exp.exp_basic import Exp_Basic
from graph_transofritto.informer.models.model import Informer, InformerStack, GATInformer
from graph_transofritto.informer.utils.metrics import metric
from graph_transofritto.informer.utils.tools import EarlyStopping, adjust_learning_rate
    ...
    def _build_model(self):
        ...
        if self.args.model in ["informer", "informerstack"]:
            ...
            model = model_dict[self.args.model](
                ...
                selected_cols=self.args.selected_cols,
                # GAT configs
                gat_layers=getattr(self.args, "gat_layers", 2),
                gat_heads=getattr(self.args, "gat_heads", 4),
                gat_k=getattr(self.args, "gat_k", 2),
                gat_hidden=getattr(self.args, "gat_hidden", None),
                gat_dropout=getattr(self.args, "gat_dropout", 0.0),
                apply_gat_to_dec=getattr(self.args, "apply_gat_to_dec", False),
            ).float()
